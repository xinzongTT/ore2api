...
class ApiVideoReferenceMediaTest(unittest.TestCase):
    def test_video_generation_accepts_image_url_alias(self) -> None:
        ...
        self.assertEqual(response.json()["data"][0]["url"], "https://cdn.oreateai.com/aivideo/unit-test-video.mp4")
    ...
    def test_video_generation_normalizes_legacy_seedance_model(self) -> None:
        app = FastAPI()
        app.include_router(ai.create_router())
        ...
        with (
            ...
            response = TestClient(app).post(
                "/v1/video/generations",
                headers={"Authorization": "Bearer test"},
                json={"prompt": "animate this", "model": "seedance-2.0"},
            )
